...
static vector<EdgeHandle> vector_tree_eh;				//最小生成树的路径
static vector<EdgeHandle> vector_all_eh;				//所有的路径 或者 所有的路径 - 最小生成树的路径
static vector<EdgeHandle> vector_false_eh;				//上一次标记为false的路径
static vector< vector<EdgeHandle> > vector_all_loop;		//所有loop的路径


void set_mesh_true(MyMesh *mesh)				//上一次被标记为true， 标记为false；
{
...
void set_loop_true(MyMesh *mesh)						//整个mesh标记为true
{
...
bool is_closed(MyMesh *mesh, EdgeHandle eh, int edge_id) //判断是否形成了一个环
{
...
void find_path(MyMesh *mesh, EdgeHandle eh, int last_id, int edge_id)//递归找最小生成树的路径
{
...
void spanning_tree(MyMesh *mesh) // 生成最小生成树
{
	int i = 0;
	int edge_id = 0;
	EdgeHandle eh;
...
bool find_loop(MyMesh *mesh, int &id)		//找到所有的环路。从中可以提取出loop。
{
...
void save_false_flag(MyMesh *mesh)
{
	...
	//cout << "\n当前找到的路径数目： " << vector_all_loop.size()<<" 长度为="<<len << endl;
}
